...
client.on('interactionCreate', async (interaction) => {
  ...
  const channel = await interaction.guild.channels.create({ name: ticketName, type: ChannelType.GuildText, topic: `ticketOwner:${interaction.user.id}; panel:Hood Support; option:${selected}`, permissionOverwrites });
  await channel.send({ content: `${interaction.user} <@&1532198278144196850>`, embeds: [welcomeEmbed] });
...
if (!token) { console.error('Missing token. Set DISCORD_TOKEN in env or put token in token.js (not recommended).'); process.exit(1); }
client.login(token);